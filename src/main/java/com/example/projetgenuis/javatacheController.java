...
public class javatacheController implements Initializable {
    ...
    private void readapprenant() {
        try (BufferedReader reader = new BufferedReader(new FileReader("./fichierjava.txt"))) {
            String line;
    ...
    private void saveapprenantinfilee(tache t) {
        ...
        byte data[] = s.getBytes();
        Path p = Paths.get("./quizesjava.txt");
        try (OutputStream out = new BufferedOutputStream(
                Files.newOutputStream(p, CREATE, APPEND))) {
            out.write(data, 0, data.length);
        } catch (IOException x) {
    ...
    private void readapprenantt() {
        Path file = Paths.get("./quizesjava.txt");
    ...
    void ajoutaches(ActionEvent event) {
       ...
       if(s>100)
            { Alert alert = new Alert(Alert.AlertType.ERROR, "somme des taches doit etre inférieur ou égal  à 100!");
               alert.showAndWait();
               return;}
