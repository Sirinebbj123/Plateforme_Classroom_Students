...
public class webtacheController implements Initializable {
    ...
    private void readapprenant() {
        try (BufferedReader reader = new BufferedReader(new FileReader("./fichierweb.txt"))) {
            String line;
    ...
    @FXML
    public void calculersommer(ActionEvent event) {
        //tache t=new tache();
        //if( this.datajt.add(tacheSelect)){
        //  t.setQuiz1(Integer.valueOf(txtaj1.getText()));
    ...
    private void saveapprenantinfilee(tache t) {
        ...
        byte data[] = s.getBytes();
        Path p = Paths.get("./quizesweb.txt");
        try (OutputStream out = new BufferedOutputStream(
                Files.newOutputStream(p, CREATE, APPEND))) {
            out.write(data, 0, data.length);
        } catch (IOException x) {
    ...
    private void readapprenantt() {
        Path file = Paths.get("./quizesweb.txt");
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        nomcol.setCellValueFactory(new PropertyValueFactory<tache, String>("nom"));
        prenomcol.setCellValueFactory(new PropertyValueFactory<tache, String>("prenom"));
        ddncol.setCellValueFactory(new PropertyValueFactory<tache, String>("datedenaissance"));
        mailcol.setCellValueFactory(new PropertyValueFactory<tache, String>("adressemail"));
